...
const DEBUG2 = false;
if (DEBUG)
    console.log("++++ channel server code loaded ++++ DEBUG is enabled ++++");
if (DEBUG2)
    console.log("++++ DEBUG2 (verbose) enabled ++++");
...
function returnResult(request, contents, status, delay = 0) {
    ...
    return new Promise((resolve) => {
        setTimeout(() => {
            if (DEBUG2)
                console.log("++++ returnResult() contents:", contents, "status:", status);
            resolve(new Response(contents, { status: status, headers: corsHeaders }));
        }, delay);
    });
}
function returnError(_request, errorString, status, delay = 0) {
    if (DEBUG)
        console.log("**** ERROR: (status: " + status + ")\n" + errorString);
    if (!delay && ((status == 401) || (status == 403)))
        delay = 50; // delay if auth-related
    ...
    try {
        ...
        if (err instanceof Error) {
            if (request.headers.get("Upgrade") == "websocket") {
                ...
                if (server.accept) {
                    server.accept(); // CF typing override (TODO: report this)
...
async function callDurableObject(name, path, request, env) {
    ...
    if (DEBUG2) {
        console.log("callDurableObject() newUrl:");
        console.log(newUrl);
    }
...
export class ChannelServer {
    ...
    #channelKeyStrings;
    room_id = '';
    room_owner = null; // duplicate, placeholder / TODO cleanup=
    ownerCalls;
    visitorCalls;
    adminCalls;
    lastTimestamp = 0; // monotonically increasing timestamp
    storageLimit = 0;
    verified_guest = '';
    visitors = [];
    join_requests = [];
    accepted_requests = [];
    lockedKeys = []; // tracks history of lock keys
    room_capacity = 20;
    ownerUnread = 0;
    locked = false;
    motd = '';
    ...
    personalRoom = false;
    motherChannel = '';
    messagesCursor = null; // used for 'startAfter' option
    ...
    async #initialize(room_id) {
        try {
            if (DEBUG)
                console.log(`==== ChannelServer.initialize() called for room: ${room_id} ====`);
            ...
            const ownerKeyJWK = jsonParseWrapper(keyStrings.ownerKey, 'L426');
            ...
            if (storageLimit === Infinity) {
                ...
                if (ledgerData) {
                    ...
                    if (DEBUG2)
                        console.log(`note that size in ledger was ${size}, in case that differs from json`);
                    this.motherChannel = mother;
                    if (DEBUG)
                        console.log(`[initialize] Found storageLimit in ledger: ${this.storageLimit}`);
                }
                else {
                    this.storageLimit = NEW_CHANNEL_BUDGET;
                    this.motherChannel = 'BOOTSTRAP';
                    if (DEBUG)
                        console.log(`++++ new channel, no SIZE provided, setting to default (${this.storageLimit / (1024 * 1024)} MiB)`);
                }
    ...
    async fetch(request) {
        ...
        if (!this.room_id) {
            ...
            if (roomId) {
                ...
                if (ret)
                    return ret; // if there was an error, return it, otherwise fall through
            }
            else {
    ...
    async #getRecentMessages(howMany, cursor = '') {
        const listOptions = { limit: howMany, prefix: this.room_id, reverse: true };
        if (cursor !== '')
        ...
        const getOptions = { allowConcurrency: true };
    ...
    #setupSession(session, msg) {
        ...
        try {
            ...
            const _name = jsonParseWrapper(data.name, 'L578');
            const isPreviousVisitor = sbCrypto.lookupKey(_name, this.visitors) >= 0;
            const isAccepted = sbCrypto.lookupKey(_name, this.accepted_requests) >= 0;
    ...
    async #handleSession(webSocket, _ip) {
        ...
        webSocket.accept(); // typing override
    ...
    async #acceptVisitor(request) {
        ...
        const acceptPubKey = jsonParseWrapper(data.pubKey, 'L783');
    ...
    #roundSize(size, roundUp = true) {
        if (size === Infinity)
            return Infinity; // special case
        if (size <= STORAGE_SIZE_MIN)
            size = STORAGE_SIZE_MIN;
        if (size > (2 ** 52))
            throw new Error(`Storage size too large (max 2^52 and we got ${size})`);
        const exp1 = Math.floor(Math.log2(size));
        const exp2 = exp1 - 3;
        const frac = Math.floor(size / (2 ** exp2));
        const result = frac << exp2;
        if ((size > result) && roundUp)
            return result + (2 ** exp2);
        else
            return result;
    }
    ...
    async #handleNewStorage(request) {
        ...
        if (size > storageLimit)
            return returnError(request, 'Not sufficient storage budget left in channel', 507);
        if (size > STORAGE_SIZE_MAX)
            return returnResult(request, `Storage size too large (max ${STORAGE_SIZE_MAX} bytes)`, 413);
    ...
    */
    async #handleBuddRequest(request) {
        ...
        if (transferBudget > this.storageLimit)
            return returnError(request, '[budd()]: Not enough storage budget in mother channel for request', 507);
        ...
        if (DEBUG)
            console.log(`[budd()]: Removing ${size} bytes from ${this.room_id.slice(0, 12)}... and forwarding to ${targetChannel.slice(0, 12)}... (new mother storage limit: ${newStorageLimit} bytes)`);
        const data = await request.arrayBuffer();
        const jsonString = new TextDecoder().decode(data);
        let jsonData = jsonString ? jsonParseWrapper(jsonString, 'L1018') : {};
        if (jsonData.hasOwnProperty("SERVER_SECRET"))
            return returnError(request, `[budd()]: SERVER_SECRET set? Huh?`, 403);
    ...
    async #verifySign(secretKey, sign, contents) {
        ...
        const verified = await crypto.subtle.verify({ name: 'ECDSA', hash: 'SHA-256' }, secretKey, _sign, encoded);
        return verified;
    }
    ...
    async #verifyAuthSign(request) {
        ...
        if (!this.#channelKeys) {
            if (DEBUG)
                console.log("verifyAuthSign(): no channel keys");
            return false;
        }
        ...
        if (new Date().getTime() - parseInt(auth_parts[0]) > 60000) {
            if (DEBUG)
                console.log("verifyAuthSign(): auth token expired");
            return false;
        }
        const sign = auth_parts[1];
    ...
    async #sendWebNotifications(message) {
        ...
        if (!envNotifications) {
            if (DEBUG)
                console.log("Cannot send web notifications (expected behavior if you're running locally");
            return;
        }
        if (DEBUG)
    ...
    async #createChannel(request) {
        ...
        for (const key of ["ownerKey", "encryptionKey", "signKey", "motherChannel", "visitors"]) {
            ...
            if (newData) {
                if (DEBUG2)
                    console.log("++ createChannel(): putting key, value: ", key, newData);
        ...
        if (DEBUG)
            console.log("CREATED channel:", this.#describe());
        return null; // null means no errors
    }
    async #getChannelKeys(request) {
        const data = {};
    ...
    async #uploadData(request) {
        ...
        if ((requestAuthorized) && (jsonData.hasOwnProperty("size")) && (targetChannel === this.room_id)) {
            ...
            await this.storage.put("storageLimit", this.storageLimit);
            if (DEBUG)
                console.log(`uploadData(): increased budget by ${this.storageLimit} bytes`);
        }
        if ((this.room_owner === jsonData["roomOwner"]) || requestAuthorized) {
            if (DEBUG)
                console.log("==== uploadData() allowed - creating a new channel ====");
            let entriesBuffer = {};
            let i = 0;
            for (const key in jsonData) {
                ...
                if (key.length != 106) {
                    if (DEBUG)
                        console.log("uploadData() key skipped on 'upload': ", key);
                }
                    ...
                    if (_key.match(/^[01]+$/)) {
                        ...
                        if (DEBUG)
                            console.log("uploadData() key not allowed (timestamp not valid): ", key);
                    }
                }
                else {
                    if (DEBUG)
                        console.log("uploadData() key not allowed (channel id not valid): ", key);
                }
            }
            ...
            if (DEBUG)
                console.log("uploadData() not allowed (room might be partially created)");
